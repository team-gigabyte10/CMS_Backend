...
class ContactController {
    ...
    static async getAllContacts(req, res, next) {
        try {
            const { page = 1, limit = 100, search = '', parent_id = null, unit_id = '', department_id = '', tree = 'true', format = 'tree', // 'tree', 'flat', 'hierarchical'
            include_inactive = 'false', role_filter = 'all' // 'all', 'super_admin', 'admin', 'user'
             } = req.query;
            const options = {
                page: parseInt(page),
    ...
    static async createContact(req, res, next) {
        try {
            ...
            const { name, rank_id, service_no, login_id, unit_id, department_id, designation_id, phone, mobile, alternative_mobile, email, parent_id, password_hash, role_id = 3, status = 'offline', avatar = null } = req.body;
            // Validate parent-child relationship
            if (parent_id) {
                const parentValidation = await ContactController.validateParentChildRelationship(parent_id, unit_id);
            ...
            const orgValidation = await ContactController.validateOrganizationalStructure(contactData.unit_id, contactData.department_id, contactData.parent_id);
        ...
        try {
            ...
            if (updateData.parent_id !== undefined) {
                if (updateData.parent_id) {
                    ...
                    // Validate parent-child relationship
                    const parentValidation = await ContactController.validateParentChildRelationship(updateData.parent_id, updateData.unit_id || contact.unit_id);
                    if (!parentValidation.valid) {
                        return res.status(400).json({
                            status: 'error',
                            message: parentValidation.message
            ...
            if (updateData.service_no || updateData.email) {
                const duplicateCheck = await ContactController.checkForDuplicatesOnUpdate(id, updateData.service_no || contact.service_no, updateData.email || contact.email, updateData.login_id || contact.login_id);
                if (duplicateCheck.exists) {
                    return res.status(400).json({
                        status: 'error',
                        message: duplicateCheck.message
    ...
    static async getContactTreeByUnit(req, res, next) {
        try {
            ...
            const { search = '', department_id = '', format = 'tree', include_inactive = 'false', role_filter = 'all' } = req.query;
            const options = {
                search,
                unit_id: unitId,
                department_id,
    ...
    static async getContactTreeByDepartment(req, res, next) {
        try {
            ...
            const { search = '', format = 'tree', include_inactive = 'false', role_filter = 'all' } = req.query;
            const options = {
                search,
        ...
        try {
            ...
            for (const id of contactIds) {
                try {
                    ...
                    // Validate parent-child relationship
                    if (parent_id) {
                        const parentValidation = await ContactController.validateParentChildRelationship(parent_id, contact.unit_id);
                        if (!parentValidation.valid) {
    ...
    static async checkForDuplicates(serviceNo, email, loginId) {
        try {
            ...
            const [serviceNoRows] = await pool.execute('SELECT id FROM users WHERE service_no = ? AND is_active = 1', [serviceNo]);
            if (serviceNoRows.length > 0) {
                return { exists: true, message: 'Service number already exists' };
            }
            // Check for duplicate email
            const [emailRows] = await pool.execute('SELECT id FROM users WHERE email = ? AND is_active = 1', [email]);
            if (emailRows.length > 0) {
                return { exists: true, message: 'Email already exists' };
            }
            // Check for duplicate login_id
            const [loginIdRows] = await pool.execute('SELECT id FROM users WHERE login_id = ? AND is_active = 1', [loginId]);
    ...
    static async checkForDuplicatesOnUpdate(contactId, serviceNo, email, loginId = null) {
        try {
            const { pool } = require('../config/database');
            // Check for duplicate service number
            const [serviceNoRows] = await pool.execute('SELECT id FROM users WHERE service_no = ? AND is_active = 1 AND id != ?', [serviceNo, contactId]);
            if (serviceNoRows.length > 0) {
                return { exists: true, message: 'Service number already exists' };
            }
            // Check for duplicate email
            const [emailRows] = await pool.execute('SELECT id FROM users WHERE email = ? AND is_active = 1 AND id != ?', [email, contactId]);
            if (emailRows.length > 0) {
                return { exists: true, message: 'Email already exists' };
            }
            // Check for duplicate login_id (if provided)
            if (loginId) {
                const [loginIdRows] = await pool.execute('SELECT id FROM users WHERE login_id = ? AND is_active = 1 AND id != ?', [loginId, contactId]);
                if (loginIdRows.length > 0) {
                    return { exists: true, message: 'Login ID already exists' };
                }
            }
    ...
    static async validateOrganizationalStructure(unitId, departmentId, parentId) {
        try {
            ...
            const [unitRows] = await pool.execute('SELECT id, name FROM units WHERE id = ? AND is_active = 1', [unitId]);
            ...
            if (departmentId) {
                const [deptRows] = await pool.execute('SELECT id, name, unit_id FROM departments WHERE id = ? AND is_active = 1', [departmentId]);
            ...
            if (parentId) {
                const [parentRows] = await pool.execute('SELECT id, name, unit_id, department_id FROM users WHERE id = ? AND is_active = 1', [parentId]);
