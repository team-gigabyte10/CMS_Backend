...
class Contact {
    ...
    static async create(contactData) {
        const { name, rank_id, service_no, login_id, unit_id, department_id, designation_id, phone, mobile, alternative_mobile, email, parent_id, password_hash, role_id = 3, status = 'offline', avatar = null } = contactData;
    ...
    static async findAll(options = {}) {
        const { page = 1, limit = 50, search = '', parent_id = null, unit_id = '', department_id = '', include_children = true } = options;
    ...
    static async getContactTree(options = {}) {
        const { search = '', unit_id = '', department_id = '', include_inactive = false, role_filter = 'all' } = options;
        try {
            let query = `
        SELECT 
          u.*,
    ...
    static async getEnhancedStatistics(options = {}) {
        const { unit_id = '', department_id = '', include_inactive = false, role_filter = 'all' } = options;
        try {
