...
class Unit {
    ...
    static async create(unitData) {
        const { name, code, description, parent_id, created_by } = unitData;
        const query = `
    ...
    static async findAll(options = {}) {
        const { page = 1, limit = 50, search = '', parent_id = null, include_inactive = false } = options;
    ...
    static async delete(id) {
        ...
        const [users] = await pool.execute('SELECT COUNT(*) as count FROM users WHERE unit_id = ? AND is_active = 1', [id]);
        if (users[0].count > 0) {
            throw new Error('Cannot delete unit with active users. Please reassign or delete users first.');
        }
        // Check if unit has departments
        const [departments] = await pool.execute('SELECT COUNT(*) as count FROM departments WHERE unit_id = ? AND is_active = 1', [id]);
        ...
        const [children] = await pool.execute('SELECT COUNT(*) as count FROM units WHERE parent_id = ? AND is_active = 1', [id]);
