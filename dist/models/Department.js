...
class Department {
    ...
    static async findAll(options = {}) {
        const { page = 1, limit = 50, search = '', unit_id = '', include_inactive = false } = options;
    ...
    static async delete(id) {
        ...
        const [users] = await pool.execute('SELECT COUNT(*) as count FROM users WHERE department_id = ? AND is_active = 1', [id]);
        if (users[0].count > 0) {
